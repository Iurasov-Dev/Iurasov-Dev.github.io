...
// ===== ИХТИС ДРОПДАУН =====
function toggleIchthys(event) {
    event.stopPropagation();
    const wrapper = event.currentTarget.closest('.ichthys-drop-wrapper');
    wrapper.classList.toggle('active');
}

// Закрытие при клике вне
document.addEventListener('click', function(e) {
    const wrappers = document.querySelectorAll('.ichthys-drop-wrapper');
...
document.addEventListener('keydown', function(e) {
    if (e.key === 'Escape') {
        const wrappers = document.querySelectorAll('.ichthys-drop-wrapper.active');
        wrappers.forEach(function(wrapper) {
            wrapper.classList.remove('active');
        });
    }
